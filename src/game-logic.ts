import { BOARD_SIZE, MovementType, PAWNS_ROWS, PawnType, PlayerType, Position, Table2d } from './shared'

interface BoardEntry extends Position {
	x: number
	y: number
	player: PlayerType
	pawn: PawnType
...
export interface PossibleMove {
	...
	from: BoardEntry
	to?: BoardEntry
	attackedPawns?: BoardEntry[]
	upgradeToKingAfterMove?: boolean
...
export class GameLogic {
	public readonly board: Table2d<BoardEntry>
	public continuationEntry: BoardEntry

	constructor() {
		this.board = new Table2d<BoardEntry>(BOARD_SIZE, BOARD_SIZE)
		this.reset()
	}
	...
	getAvailableMoves(meX: number, meY: number, playerType: PlayerType): PossibleMove[] {
	...
	private getAvailableMovesForNormalPawn(args: CheckMovesArgs) {
		...
		const checkPossibility = (dirX, dirY, attackOnly, dontAdd = false) => {
			...
			if (tile) {
				if (!attackOnly && tile.pawn === 'none') {
					if (dontAdd || args.isBound) return false
	...
	private getAvailableMovesForKing(args: CheckMovesArgs) {

		const checkRow = (dirX: number, dirY: number) => {
			let x = args.meX
			let y = args.meY
			let attacked: BoardEntry[] = []
