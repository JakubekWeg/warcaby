...
import { BOARD_SIZE, FAST_TEST_ENABLED, MovementType, PawnType, PlayerType, TILE_SIZE } from './shared'
...
export const pawnIndexToPosition = (index: number): number => TILE_SIZE / 2 + index * TILE_SIZE - TILE_SIZE * BOARD_SIZE / 2

export class Pawn {
	public readonly object3d: THREE.Object3D
	private isGeneratingFlames: boolean
	private crystal: EndCrystal = null
	private beam: EndCrystalBeam = null

	constructor(
		private game: FrontEndGameInstance,
		...
		switch (type) {
			...
			case 'pawn':
				materialIndex = {
					['black']: materialIndex = MaterialType.BlackPawn,
					['white']: materialIndex = MaterialType.WhitePawn,
				}[player]
				break
		}
	...
	public goToPosition(boardX: number, boardY: number, movementType: MovementType) {
		const start = new THREE.Vector3(
			pawnIndexToPosition(this.boardX),
			DEFAULT_PAWN_Y,
			pawnIndexToPosition(this.boardY),
		)
		const end = new THREE.Vector3(
			pawnIndexToPosition(boardX),
			DEFAULT_PAWN_Y + (PAWN_HEIGHT * (movementType !== 'movement' ? 3 : 2)),
			pawnIndexToPosition(boardY),
		)
	...
	public goToPositionAfterKilling(destinationX: number, destinationY: number,
	                                lastKillX: number, lastKillY: number) {
	...
	public playDeathByPawnAnimation(instant: boolean = false) {
		...
		setTimeout(() => {
			...
			for (let i = 0; i < count; i++) {
				this.game.particles.poof.spawn(
					spawnParticlesLocationX + THREE.MathUtils.randFloat(-TILE_SIZE, TILE_SIZE),
					THREE.MathUtils.randFloat(-10, TILE_SIZE),
					spawnParticlesLocationZ + THREE.MathUtils.randFloat(-TILE_SIZE, TILE_SIZE),
					0,
				)
			}
	...
	public playDeathByKingAnimation() {
		...
		setTimeout(() => {
			const damageAnimation = new FrameAnimation(300, value => {
				...
				const obj = <THREE.Mesh>this.object3d
	...
	public upgradeToKing() {
		...
		setTimeout(() => {
			...
			new FrameAnimation(FAST_TEST_ENABLED ? 0 : 2000, value => {
				this.game.particles.glint.spawn(
					x + THREE.MathUtils.randFloat(-10, +10),
					DEFAULT_PAWN_Y + Math.random() * 10,
				...
				for (let i = 0; i < 100; i++)
					this.game.particles.poof.spawn(
						x + THREE.MathUtils.randFloat(-TILE_SIZE, TILE_SIZE),
						DEFAULT_PAWN_Y + Math.random() * TILE_SIZE * 1.5,
						z + THREE.MathUtils.randFloat(-TILE_SIZE, TILE_SIZE),
						0,
					)
	...
	update(delta: number, elapsedTime: number) {
		...
		if (this.isGeneratingFlames) {
			...
			this.game.particles.flame.spawn(
