...
export class BreakingBlockAnimation {
    constructor(_canvas, _block, _pick) {
    ...
    breakBlock() {
        if (!this._running || this._brokeBlock)
            return;
    ...
    _renderParticles() {
        for (const p of this._particles) {
            ...
            this._ctx.drawImage(this._block, p.tx, p.ty, 2, 2, p.x, p.y, 2 * SCALE, 2 * SCALE);
        }
    }
    _render() {
        if (!this._running)
