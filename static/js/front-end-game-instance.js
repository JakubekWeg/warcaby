...
import { commonMaterials, LIGHT_ENABLED, RENDER_IF_NOT_FOCUS } from './globals-3d';
...
import { Table2d, TILE_SIZE } from './shared';
...
class SelectedPawnController {
    constructor(scene) {
        ...
        this.material = commonMaterials[11 /* EnchantedItem */];
        const offset1 = .1;
    ...
    update() {
        ...
        for (const face of this.mesh1.geometry.faceVertexUvs[0])
            for (const uv of face) {
                uv.x -= .0018;
                uv.y -= .0023;
            }
        this.mesh1.geometry.uvsNeedUpdate = true;
        for (const face of this.mesh2.geometry.faceVertexUvs[0])
            for (const uv of face) {
                uv.x -= .0007;
                uv.y += .0016;
            }
        this.mesh2.geometry.uvsNeedUpdate = true;
    }
}
export class FrontEndGameInstance {
    ...
    constructor() {
        ...
        this.particles = new ParticlesEngine(this.scene);
        this.pawnsList = [];
        this.frameClock = new THREE.Clock();
        this.rayCaster = new THREE.Raycaster();
        this.canvas = document.getElementById('3d-game');
        this.renderFunction = this.render.bind(this);
        this.windowHasFocus = true; /*document.hasFocus()*/
        this.logic = new GameLogic();
        this.pawnsTable = new Table2d(BOARD_ITEMS_COUNT, BOARD_ITEMS_COUNT);
        this.selected = new SelectedPawnController(this.scene);
        this.possibleMovesBoxes = [];
        ...
        this.networking.addEventListener('game-finished', ({ reason, won, lost }) => {
            if (this.networking.myType === won)
                showBigToast('You won!', 100000);
            else if (this.networking.myType === lost)
                showBigToast('You lost!', 100000);
            showSubToast(`Game finished: ${reason}`, 5000);
    ...
    getTileByMousePointer({ offsetX, offsetY }) {
    ...
    selectPawnIfRequired(forceHideMessage = false) {
        if (this.networking.isMyTurn && this.logic.continuationEntry) {
            ...
            if (!tile)
                alert('Unable to select required pawn');
            ...
            if (result !== null) {
                if (result.move) {
                    ...
                    if (result.pawn.player === this.networking.myType) {
                        ...
                        for (let i = 0; i < 3; i++) {
                            this.particles.angry.spawn(pos.x + THREE.MathUtils.randFloat(-spread, spread), pos.y + THREE.MathUtils.randFloat(-spread, spread), pos.z + THREE.MathUtils.randFloat(-spread, spread), Date.now() + THREE.MathUtils.randFloat(400, 2000));
                        }
                    }
                }
            }
    ...
    translateTouchEvent(event, call) {
        const touch = event.touches[0] || event.changedTouches[0];
        if (touch) {
            call({
                offsetX: (touch.pageX - this.canvas.offsetLeft) * window.devicePixelRatio,
                offsetY: (touch.pageY - this.canvas.offsetTop) * window.devicePixelRatio,
            });
        }
    }
    onMouseMoved(event) {
        if (!this.windowHasFocus && !RENDER_IF_NOT_FOCUS || !this.board)
            return;
        ...
        if (result) {
            if (result.pawn) {
                ...
                this.hoverPawnBox.position.set(pawnIndexToPosition(result.indexX), DEFAULT_PAWN_Y + .5, pawnIndexToPosition(result.indexZ));
                this.hoverPawnBox.updateMatrix();
            }
            else if (result.move) {
                this.hoverPawnBox.visible = false;
                this.hoverTileBox.visible = false;
                this.hoveredPossibleMove = new THREE.Vector3(pawnIndexToPosition(result.move.to.x), DEFAULT_PAWN_Y, pawnIndexToPosition(result.move.to.y));
            }
            else {
                this.hoveredPossibleMove = null;
                this.hoverPawnBox.visible = false;
                this.hoverTileBox.visible = true;
                this.hoverTileBox.position.set(pawnIndexToPosition(result.indexX), PAWN_HEIGHT * 1.5 / 2 + .5, pawnIndexToPosition(result.indexZ));
    ...
    showPossibleMoves(moves) {
        ...
        if (moves) {
            ...
            for (const move of moves) {
                ...
                if (!lines) {
                    lines = new THREE.LineSegments(pawnEdgesGeometry, commonMaterials[12 /* BoxOutline */]);
                    lines.matrixAutoUpdate = false;
                    this.scene.add(lines);
                    this.possibleMovesBoxes.push(lines);
                }
                lines.visible = true;
                lines.position.set(pawnIndexToPosition(move.to.x), DEFAULT_PAWN_Y, pawnIndexToPosition(move.to.y));
                lines.updateMatrix();
            }
        }
    }
    ...
    setUp3dScene() {
        ...
        {
            this.hoverPawnBox = new THREE.LineSegments(pawnEdgesGeometry, commonMaterials[12 /* BoxOutline */]);
        ...
        }
        {
            ...
            this.hoverTileBox = new THREE.LineSegments(edges, commonMaterials[12 /* BoxOutline */]);
    ...
    updateCameraAspect() {
        ...
        this.renderer.setSize(w * multiplier | 0, h * multiplier | 0, false);
    ...
    render() {
        if (this.windowHasFocus || RENDER_IF_NOT_FOCUS) {
            ...
            if (this.hoveredPossibleMove && Math.random() > .95) {
                this.particles.glint.spawn(this.hoveredPossibleMove.x + THREE.MathUtils.randFloat(-10, 10), this.hoveredPossibleMove.y + THREE.MathUtils.randFloat(0, 10), this.hoveredPossibleMove.z + THREE.MathUtils.randFloat(-10, 10), Date.now() + Math.random() * 1000 + 2000);
            }
