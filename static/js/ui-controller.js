export const setNicksAndColors = (nick1, color1, nick2, color2) => {
...
let timerExpires = 0;
export const runTimer = (expires) => {
    timer.style.display = null;
    timerExpires = expires;
};
export const hideTimer = () => (timer.style.display = 'none') && (timerExpires = 0);
setInterval(() => timerExpires && (timer.innerText = `${Math.max((timerExpires - Date.now()) / 1000 | 0, 0)}s`), 1000);
const mainText = {
    timeoutId: null,
    allowShowAt: 0,
    hiding: false,
...
const showText = (obj, message, timeout) => {
    const now = Date.now();
...
export const showBigToast = (message, timeout = 3000) => showText(mainText, message, timeout);
export const showSubToast = (message, timeout = 3000) => showText(subText, message, timeout);
